...
const RecentGraduates = () => {
  ...
  useEffect(() => {
    const fetchRecent = async () => {
      try {
        const { data } = await graduateAPI.getRecent();
        setGraduates(data);
      } catch {
        setGraduates([]);
      } finally {
      ...
      <div className="container-custom">
        ...
        >
          <div>
            ...
            <p className="text-gray-500 text-lg">
              The latest graduates to join the archive
            </p>
          </div>
          <div className="hidden md:flex items-center space-x-3">
            <button
              onClick={() => scroll('left')}
              className="w-12 h-12 rounded-xl bg-white shadow-lg hover:shadow-xl flex items-center justify-center text-gray-600 hover:text-primary-600 transition-all duration-300 border border-gray-100"
            >
            ...
            <Link
              to="/archive"
              className="px-5 py-2.5 text-sm font-medium text-primary-600 hover:text-primary-700 bg-primary-50 hover:bg-primary-100 rounded-xl transition-all duration-300 flex items-center space-x-1"
            >
              <span>View All</span>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Be the first to join the archive"
          />
        ) : (
          <>
            <div
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {grad.quote && (
                            <span className="text-xs text-gray-400 italic truncate ml-2">
                              &ldquo;{grad.quote.substring(0, 25)}&rdquo;
                            </span>
                          )}
                        </div>
